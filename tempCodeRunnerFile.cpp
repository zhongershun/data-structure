...
public:
    ...
    void build_preorder(Bino_node<entry>* &aroot,char str[],int &index);
};
...
template<class entry>
void Bino_tree<entry>::build_preorder(Bino_node<entry>* &aroot,char str[],int &index)
{
    if (index<strlength)
    {
        if (str[index]=='#')
...
{
    bool can=false;
        Bino_node<char> *nodel=node->left;
        Bino_node<char> *noder=node->right;
        int hl=height(nodel);
        int hr=height(noder);
        if ((hl-hr)>=-1&&(hl-hr)<=1)
        {
